...
# --- NumPy 2 compatibility shim for old SciPy / dt_state_estimation ---
...
# np.typeDict was removed; map it to sctypeDict if available
if not hasattr(_np, "typeDict"):
    if hasattr(_np, "sctypeDict"):
        _np.typeDict = _np.sctypeDict  # type: ignore[attr-defined]
    else:
        # Fallback: build a minimal mapping (rarely needed)
        _np.typeDict = {}  # type: ignore[attr-defined]
# ---------------------------------------------------------------------
...
class LaneFilterNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        try:
            ...
            self._filter['wheel_baseline'] = rospy.get_param("kinematics_node/baseline", 0.1)
            self._filter['wheel_radius'] = rospy.get_param("kinematics_node/radius", 0.0318)
